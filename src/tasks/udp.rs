...
pub async fn udp_receiver(
    ...
    loop {
        udp.with_packet(async |(addr, packet)| {
            ...
            if let Some(msg) = packet.into_iter().next() {
                match postcard::from_bytes::<NetworkMessage>(msg) {
                    Ok(network_msg) => match network_msg {
                        ...
                        NetworkMessage::Ledger(sent_ledger) => {
...
pub async fn manage_ledger(
    ...
    loop {
        ...
        let book = address_book.lock().await;
        let mut ledge = ledger.lock().await;

        if book.is_empty() && ledge.is_none() {
            log_info!("Creating new ledger");
            let new_ledger = Ledger {
                owner: my_addr,
                jobs: FnvIndexSet::new(),
            };
            *ledge = Some(new_ledger);
        }
        ...
        if let Some(mut l) = ledge.take() {
            ...
            if marlin::is_ready() && marlin::is_idle() {
                ...
                for job_guid in l.jobs.iter() {
                    let path = make_path(job_guid, false);
