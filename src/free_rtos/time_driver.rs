...
pub struct FreeRtosTimeDriver {
    pub timekeeper: critical_section::Mutex<Cell<u64>>,
    pub free_rtos_now: critical_section::Mutex<Cell<u32>>,
    pub queue: critical_section::Mutex<RefCell<Queue>>,
}
...
impl Driver for FreeRtosTimeDriver {
    ...
    fn now(&self) -> u64 {
        critical_section::with(|cs| {
            ...
            let prev = self.free_rtos_now.borrow(cs).get();
            let tick_diff = free_rtos_now.wrapping_sub(prev);
            if tick_diff > 0 {
                self.free_rtos_now.borrow(cs).set(free_rtos_now);
                let tk = self.timekeeper.borrow(cs);
                tk.set(tk.get() + tick_diff as u64);
            }
            self.timekeeper.borrow(cs).get()
        })
    }
